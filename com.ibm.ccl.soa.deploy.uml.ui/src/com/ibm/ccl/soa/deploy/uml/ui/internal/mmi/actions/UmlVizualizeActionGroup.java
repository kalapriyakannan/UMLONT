...
public class UmlVizualizeActionGroup extends ActionGroup {
	 ...
	 * @param viewSite
	 */
	public UmlVizualizeActionGroup(IWorkbenchPartSite viewSite) {
	...
	public void fillContextMenu(IMenuManager menu) {
		...
		IMenuManager submenu = new MenuManager(UmlUIMessages.UmlVizualizeActionGroup_Visualiz_,
				"deploy.vizualize.submenu"); //$NON-NLS-1$
		submenu.add(new AddToCurrentTopolgoyDiagramWrapperAction(selection));
		menu.appendToGroup("deploy.vizualize.seperator", submenu); //$NON-NLS-1$

		IMenuManager addDiagramSubMenu = new MenuManager(
				UmlUIMessages.UmlVizualizeActionGroup_Add_to_New_Diagram_Fil_,
				"deploy.vizualize.submenu2"); //$NON-NLS-1$
		addDiagramSubMenu.add(new AddToNewSequenceDiagramWrapperAction(selection));
		addDiagramSubMenu.add(new AddToNewTopologyDiagramWrapperAction(selection));
		submenu.add(addDiagramSubMenu);
