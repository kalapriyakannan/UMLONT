 ...
 */
public class GenericStackRealizationDownSatisfactionResolutionGenerator extends
	...
	public IDeployResolution[] getResolutions(IDeployResolutionContext context) {
		...
		for (Iterator<Unit> iter = matches.iterator(); iter.hasNext();) {
			...
			if (linkDescriptors.length > 0) {
				...
				Map<Unit, Unit> srMap = GenericStackRealizationDownSatisfactionResolution
						.stackRealizationMap(ld.getSourceUnit(), ld.getTargetUnit(), context);
				//				long time = System.currentTimeMillis();
				//				System.out.println(srcUnit.titleWithContext() + " -> " + candidate.titleWithContext() + //$NON-NLS-1$
				//						" : ms for GenericStackRealizationDownSatisfactionResolutionGeneration = " //$NON-NLS-1$
				//						+ (time - prevTime) + " : " + (srMap.size() > 0 ? "Success" : "Failure")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				if (srMap.size() > 1) {
					if (ld.getType().equals(LinkType.REALIZATION)) {
						...
						resolutionsList.add(new GenericStackRealizationDownSatisfactionResolution(
								context, this, srcUnit, candidate, srMap, /* candidate, */description));
					}
				}
			}
		}
	...
	private List<Unit> calculateMatches(Unit srcUnit, IDeployResolutionContext context) {
		...
		// HashSet would make this non-deterministic
		...
		for (Iterator<Unit> it = top.findAllUnits(); it.hasNext();) {
			...
			if (candidate != srcUnit) {
				Unit candidateFR = (Unit) RealizationLinkUtil.getFinalRealization(candidate);
				if (retVal.contains(candidateFR)) {
					continue;
				}

				LinkDescriptor[] linkDescriptorArray = RealizationPatternCacheService.INSTANCE
						.getPossibleLinks(srcUnit, candidateFR, linkTypes, dvs);
				if (linkDescriptorArray.length > 0) {
					retVal.add(candidateFR);
					candidateToLDs.put(candidateFR, linkDescriptorArray);
				}
			}
		}
