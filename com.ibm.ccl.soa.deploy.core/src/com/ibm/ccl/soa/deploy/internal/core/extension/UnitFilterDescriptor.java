...
/**
 * Provides a wrapper around <b>&lt;unitFilter /&gt;</b> configuration elements parsed from the
 * <b>com.ibm.ccl.soa.deploy.core.unitLifecycle</b> extension point.
 * 
 * @since 1.0
 * 
 */
public class UnitFilterDescriptor extends CommonDescriptor {
	 ...
	 * 
	 * @param anElement
	 *           The "unitFilter" IConfigurationElement from the registry reader.
	 */
	protected UnitFilterDescriptor(IConfigurationElement anElement) {
		super(anElement);
		assert TAG_UNIT_FILTER.equals(anElement.getName());
		init();
	}
	...
	private void init() {
		...
		if (inputEnablement.length == 1) {
			 ...
			 * DeployCoreMessages.UnitFilterDescriptor_Exactly_one_enablement_element_is, new Object[] {
			 * TAG_UNIT_FILTER, getName(), getElement().getDeclaringExtension()
	 ...
	 * 
	 * @return An instance of the {@link UnitFilter} defined by the extension or a no-op singleton if
	 *         a problem occurs.
	 */
	public UnitFilter createUnitFilter() {
		...
		try {
			...
			if (provider instanceof UnitFilter)
				return (UnitFilter) provider;

			String errMsg = NLS.bind(
					DeployCoreMessages.UnitFilterDescriptor_An_instance_of_UnitFilter_is_required,
					new Object[] { UnitFilter.class.getName(), ATT_CLASS,
		...
		return SkeletonUnitFilter.INSTANCE;
	}
	 ...
	 *         the extension defined by this {@link UnitFilterDescriptor}.
	 */
	...
	public String toString() {
		return "UnitFilterDescriptor[name=" + getName() + ", inputExpression=" + inputExpression + "]" + ", outputExpression=" + outputExpression + "]"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
	}
}
