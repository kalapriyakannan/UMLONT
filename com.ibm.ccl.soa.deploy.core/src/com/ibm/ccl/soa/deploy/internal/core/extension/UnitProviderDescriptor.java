...
/**
 * Provides a wrapper around <b>&lt;unitProvider /&gt;</b> configuration elements parsed from the
...
public class UnitProviderDescriptor extends CommonDescriptor {
	 ...
	 * 
	 * @param anElement
	 *           The "unitProvider" IConfigurationElement from the registry reader.
	 */
	protected UnitProviderDescriptor(IConfigurationElement anElement) {
		super(anElement);
		assert TAG_UNIT_PROVIDER.equals(anElement.getName());
	...
	private void init() {
		 ...
		 * DeployCoreMessages.UnitProviderDescriptor_Exactly_one_enablement_element_is, new
		 * Object[] { TAG_UNIT_PROVIDER, getName(), getElement().getDeclaringExtension()
	 ...
	 * 
	 * @return An instance of the {@link UnitProvider} defined by the extension or a no-op singleton
	 *         if a problem occurs.
	 */
	public UnitProvider createUnitProvider() {
		...
		try {
			...
			if (provider instanceof UnitProvider) {
				((UnitProvider) provider).setDescriptor(this);
				return (UnitProvider) provider;
			}

			String errMsg = NLS
					.bind(
							DeployCoreMessages.UnitProviderDescriptor_An_instance_of_TopologyUnitProvider_is_required,
							new Object[] { UnitProvider.class.getName(), ATT_CLASS,
	...
	public String toString() {
		return "UnitProviderDescriptor[name=" + getName() + ", inputExpression=" + inputExpression + "]" + ", outputExpression=" + outputExpression + "]"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
	}
