 ...
 * 
 */
public class ReflectionRefactoringProcessor extends RefactoringProcessor {

	private static final String PROCESSOR_ID = ReflectionRefactoringProcessor.class.getName();
	...
	private Set reflectingProviders;
	 ...
	 * @param datamodel
	 */
	public ReflectionRefactoringProcessor(SynchronizationDatamodel datamodel) {
	...
	public RefactoringStatus checkFinalConditions(IProgressMonitor monitor,
		...
		try {
			...
			for (int i = 0; i < units.length; i++) {
				...
				Set<ChangeProvider> providers = getReflectingProviders(units[i]);
	...
	public Change createChange(IProgressMonitor monitor) throws OperationCanceledException {
		...
		try {
			...
			for (int i = 0; i < units.length; i++) {
				...
				for (Iterator iter = getReflectingProviders(units[i]).iterator(); iter.hasNext();) {
					monitor.subTask(units[i].getDisplayName());
					ChangeProvider provider = (ChangeProvider) iter.next();
	...
	public String getProcessorName() {
		return DeployCoreMessages.ReflectionRefactoringProcessor_Component_Reflection_Processo_;
	}
	...
	protected final Set<ChangeProvider> getReflectingProviders(Unit component) {
		...
		Set<ChangeProvider> providers = new TreeSet<ChangeProvider>(new ChangeProviderCompator());
	 ...
	 * 
	 */
	private abstract class SynchronizationProviderRunnable extends DeployCoreSafeRunnable {
		 ...
		 */
		public SynchronizationProviderRunnable(Unit unit, IProgressMonitor monitor) {
	...
	private class CreateChangeRunnable extends SynchronizationProviderRunnable {
		...
		public void run() throws Exception {
			...
			if (cProvider != null && cProvider.canReflect()) {
				Change[] changes = cProvider.createReflectionChange(getUnit(),
	...
	private class CheckConditionsRunnable extends SynchronizationProviderRunnable {
		...
		public void run() throws Exception {
			ChangeProvider cProvider = getProvider();
			if (cProvider != null && cProvider.canReflect()) {
				RefactoringStatus status = cProvider.checkConditionsForReflection(getUnit(),
						createSubProgressMonitor());
				if (status != null) {
