 ...
 * @since 1.0
 */
public class AnalysisDomainUnitFilterTest extends TestCase {

	private static final String PROJECT_NAME = "AnalysisDomainUnitFilterTest";
	...
	public void testComponentUnitFilter() throws IOException {
		 ...
		 * Setup
		 */
		URI uri = URI.createPlatformResourceURI(PROJECT_NAME + "/testComponentUnitFilter.topology");
		...
		top.setName("TestComponentUnitFilter Topology");
	...
	public void testLocationUnitFilter() throws IOException {
		/**
		 * Setup
		 */
		URI uri = URI.createPlatformResourceURI(PROJECT_NAME + "/testLocationUnitFilter.topology");
		Resource res = WorkbenchResourceHelperBase.createResource(uri);
		DeployCoreRoot root = CoreFactory.eINSTANCE.createDeployCoreRoot();
		res.getContents().add(root);
		Topology top = CoreFactory.eINSTANCE.createTopology();
		top.setName("TestLocationUnitFilter Topology");
		...
		UnitFilterDescriptor[] enabledFilters = UnitLifeCycleManager.getInstance()
		.findEnabledTopologyUnitFiltersByInputOnly(location);
		List allIDs = new ArrayList();
		for (int i = 0; enabledFilters != null && i < enabledFilters.length; i++) {
			UnitFilter filter = enabledFilters[i].createUnitFilter();
			List<?> unitElementIDs = filter.getAllowableMemberUnitTypes(location);	
			if(unitElementIDs != null)
				allIDs.addAll(unitElementIDs);
		}
		assert allIDs.contains(IAnalysisTemplateConstants.NODE_UNIT) == true;
	...
	public void testNodeUnitFilter() throws IOException {
		URI uri = URI.createPlatformResourceURI(PROJECT_NAME + "/testNodeUnitFilter.topology");
		Resource res = WorkbenchResourceHelperBase.createResource(uri);
		DeployCoreRoot root = CoreFactory.eINSTANCE.createDeployCoreRoot();
		res.getContents().add(root);
		Topology top = CoreFactory.eINSTANCE.createTopology();
		top.setName("TestNodeUnitFilter Topology");
