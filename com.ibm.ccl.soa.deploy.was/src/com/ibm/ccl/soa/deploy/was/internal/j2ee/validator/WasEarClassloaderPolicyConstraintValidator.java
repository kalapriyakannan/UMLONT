...
public class WasEarClassloaderPolicyConstraintValidator extends ConstraintValidator {

	protected boolean isValidPlacement(DeployModelObject dmo) {
		if (!J2eePackage.Literals.EAR_MODULE.isSuperTypeOf(dmo.getEObject().eClass())) {
			return false;
		}
	...
	public boolean canValidateConstraint(Constraint constraint) {
		return WasPackage.Literals.WAS_EAR_CLASSLOADER_POLICY_CONSTRAINT.isSuperTypeOf(constraint
				.getEObject().eClass());
	}
	...
	public IStatus validate(Constraint constraint, DeployModelObject context,
		...
		// we know that it is a WasEarClassloaderPolicyConstraint
		WasEarClassloaderPolicyConstraint wasConstraint = (WasEarClassloaderPolicyConstraint) constraint;
		...
		if (!isValidPlacement(context)) {
			IDeployStatus status = DeployCoreStatusFactory.INSTANCE
					.createDeployStatus(
							IStatus.WARNING,
							IWasValidatorID.WAS_EAR_CLASSLOADER_POLICY_CONSTRAINT_INVALID_CONTEXT_VALIDATOR_001,
							IWASProblemType.WAS_EAR_CLASSLOADER_POLICY_CONSTRAINT_INVALID_CONTEXT,
