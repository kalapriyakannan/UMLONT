...
public class TypeInfoFactory {
	...
	public TypeInfo create(char[] packageName, char[] typeName, char[][] enclosingName,
		...
		if (index != -1) {
			result = createJarFileEntryTypeInfo(pn, tn, enclosingName, modifiers, path,
					getJarFileEntryTypeInfo(fLast), index);
		} else {
			String project = getProject(path);
			if (project != null) {
				result = createIFileTypeInfo(pn, tn, enclosingName, modifiers, path,
						getIFileTypeInfo(fLast), project);
			}
		}
		if (result == null) {
	...
	private TypeInfo createJarFileEntryTypeInfo(String packageName, String typeName,
			char[][] enclosingName, int modifiers, String path, JarFileEntryTypeInfo last, int index) {
		...
		return new JarFileEntryTypeInfo(packageName, typeName, enclosingName, modifiers, jar, file,
				extension);
	}

	private TypeInfo createIFileTypeInfo(String packageName, String typeName,
			char[][] enclosingName, int modifiers, String path, IFileTypeInfo last, String project) {
		...
		return new IFileTypeInfo(packageName, typeName, enclosingName, modifiers, project, src, file,
				extension);
	}
