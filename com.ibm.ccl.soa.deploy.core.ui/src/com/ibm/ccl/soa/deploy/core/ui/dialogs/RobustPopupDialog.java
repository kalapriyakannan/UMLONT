 ...
 * 
 * Contributors: IBM Corporation - initial API and implementation Stefan Xenos, IBM - bug 156790:
 * Adopt GridLayoutFactory within JFace
 **************************************************************************************************/
...
/**
 * A lightweight, transient dialog that is popped up to show contextual or temporal information and
 * is easily dismissed. Clients control whether the dialog should be able to receive input focus. An
 * optional title area at the top and an optional info area at the bottom can be used to provide
 * additional information.
 * <p>
 * Because the dialog is short-lived, most of the configuration of the dialog is done in the
 * constructor. Set methods are only provided for those values that are expected to be dynamically
 * computed based on a particular instance's internal state.
 * <p>
 * Clients are expected to override the creation of the main dialog area, and may optionally
 * override the creation of the title area and info area in order to add content. In general,
 * however, the creation of stylistic features, such as the dialog menu, separator styles, and
 * fonts, is kept private so that all popup dialogs will have a similar appearance.
 * 
 * @since 3.2
 */
public class RobustPopupDialog extends Window {
	...
	/////// ADDED ///////////
	private final MouseListener closeListener = new MouseAdapter() {
		@Override
	...
	private static final GridLayoutFactory POPUP_LAYOUT_FACTORY = GridLayoutFactory.fillDefaults()
			.margins(POPUP_MARGINWIDTH, POPUP_MARGINHEIGHT).spacing(POPUP_HORIZONTALSPACING,
	...
	/**
	 * Labels that contain title and info text. Cached so they can be updated dynamically if
	 * possible.
	 */
	...
	/**
	 * Flag specifying whether a menu action allowing the user to choose whether the dialog bounds
	 * should be persisted is to be shown.
	 */
	private boolean showPersistAction = false;

	/**
	 * Flag specifying whether the bounds of the popup should be persisted. This flag is updated by a
	 * menu if the menu is shown.
	 */
	...
	/**
	 * Control used for the OK action.
	 */
	protected Control _applyControl;

	/**
	 * The Composite that contains the _applyControl.
	 */
	 ...
	 * @param persistBounds
	 *           A boolean indicating whether the bounds should be persisted upon close of the
	 *           dialog. The bounds can only be persisted if the dialog settings for persisting the
	 *           bounds are also specified. If a menu action will be provided that allows the user to
	 *           control this feature, then the last known value of the user's setting will be used
	 *           instead of this flag.
	 * @param showDialogMenu
	 *           A boolean indicating whether a menu for moving and resizing the popup should be
	 *           provided.
	 ...
	 * @param titleText
	 *           Text to be shown in an upper title area, or <code>null</code> if there is no
	 *           title.
	 * @param infoText
	 *           Text to be shown in a lower info area, or <code>null</code> if there is no info
	 *           area.
	 * 
	 * @see PopupDialog#getDialogSettings()
	 */
	public RobustPopupDialog(Shell parent, int shellStyle, boolean takeFocusOnOpen,
	...
	protected void configureShell(Shell shell) {
		...
		shell.addListener(SWT.Deactivate, new Listener() {
			public void handleEvent(Event event) {
				/*
				 * Close if we are deactivating and have no child shells. If we have child shells, we
				 * are deactivating due to their opening. On X, we receive this when a menu child (such
				 * as the system menu) of the shell opens, but I have not found a way to distinguish
				 * that case here. Hence bug #113577 still exists.
				 */
				if (listenToDeactivate && event.widget == getShell()
					...
					/*
					 * We typically ignore deactivates to work around platform-specific event ordering.
					 * Now that we've ignored whatever we were supposed to, start listening to
	...
	/**
	 * The <code>PopupDialog</code> implementation of this <code>Window</code> method creates and
	 ...
	 * <code>createInfoTextArea</code> methods to create an optional title and menu area on the
	 * top, a dialog area in the center, and an optional info text area at the bottom. Overriding
	...
	protected Control createContents(Composite parent) {
		...
		composite.setSize(getInitialSize());
	 ...
	 * info text area.
	 * <p>
	 * The <code>PopupDialog</code> implementation of this framework method creates and returns a
	 * new <code>Composite</code> with standard margins and spacing.
	 * <p>
	 * The returned control's layout data must be an instance of <code>GridData</code>. This
	 * method must not modify the parent's layout.
	 * <p>
	 ...
	 * 
	 * @param composite
	 *           the composite in which all content, including the title area and info area, was
	 *           created. This composite's parent is the shell.
	 */
	 ...
	 * menu or title text.
	 * 
	 * @return <code>true</code> if a title area should be created, <code>false</code> if it
	 *         should not.
	 */
	...
	/**
	 * Creates the control to be used to represent the dialog's title text. Subclasses may override
	 * if a different control is desired for representing the title text, or if something different
	 * than the title should be displayed in location where the title text typically is shown.
	 * 
	 * <p>
	 * If this method is overridden, the returned control's layout data must be an instance of
	 * <code>GridData</code>. This method must not modify the parent's layout.
	...
	protected Control createInfoTextArea(Composite parent) {
		DeployUIWidgetFactory factory = DeployUIWidgetFactory.INSTANCE;
		if (requiresApplyControl()) {
			applyAreaComposite = factory.createModifiedDialogStatusBar(parent, 3);
		} else {
			applyAreaComposite = factory.createModifiedDialogStatusBar(parent, 2);
		}
		//Add info text label.
		Label info = (Label) createInfoTextLabel(applyAreaComposite);
		//Switch alignment to the left.
		info.setAlignment(SWT.LEFT);

		if (requiresApplyControl()) {
			_applyControl = factory.createOKHyperlink(applyAreaComposite, new Runnable() {
				public void run() {
					apply();
				}
			}, this);
			_applyControl.setEnabled(false);
			((Hyperlink) _applyControl).setText(getApplyControlText());
			//Add Cancel control
			factory.createCloseHyperlink(applyAreaComposite, Messages.RobustPopupDialog_Cance_, false,
					this);
		} else {
			//Add Close control
			factory.createCloseHyperlink(applyAreaComposite, true, this);
		}
		return info;
	}

	/**
	 * @return A String for the text of the apply control, if needed.
	 */
	protected String getApplyControlText() {
		return Messages.RobustPopupDialog_Appl_;
	}

	/**
	 * Subclasses should override to return false if an Apply action is not required.
	 * 
	 * @return boolean
	 */
	 ...
	 * @return
	 */
	private Control createInfoTextLabel(Composite parent) {
		// Status label
		...
		GridDataFactory.fillDefaults().grab(true, false).align(SWT.FILL, SWT.BEGINNING).applyTo(
				infoLabel);
		infoLabel.setForeground(parent.getDisplay().getSystemColor(SWT.COLOR_WIDGET_DARK_SHADOW));
		return infoLabel;
	}
	...
	private void createDialogMenu(Composite parent) {
		...
		menuImage = ImageDescriptor
				.createFromFile(PopupDialog.class, "images/popup_menu.gif").createImage();//$NON-NLS-1$
	...
	/**
	 * Show the dialog's menu. This message has no effect if the receiver was not configured to show
	 * a menu. Clients may call this method in order to trigger the menu via keystrokes or other
	 ...
	 * 
	 * @return <true> if the dialogs bounds will be persisted, false if it will not.
	 */
	...
	public int open() {
		...
		if (shell.getParent() != null) {
			shell.getParent().addMouseListener(closeListener);
		} else {
			shell.addMouseListener(closeListener);
		}
		return super.open();
	}
...
//	 * open, immediately returning <code>OK</code> if the open is successful, or
//	 * <code>CANCEL</code> if it is not. It provides framework hooks that allow subclasses to set
//	 * the focus and tab order, and avoids the use of <code>shell.open()</code> in cases where the
//	 * focus should not be given to the shell initially.
//	 * 
...
//	}
	 ...
	 * widgets can be recreated if the dialog is reopened. This method may be extended (<code>super.close</code>
	 * must be called).
	 * </p>
	 * 
	...
	public boolean close() {
		...
		/////// ADDED //////////
		getShell().removeMouseListener(closeListener);
	 ...
	 * <code>super.saveDialogBounds</code> if additional bounds information should be stored.
	 * Clients may also call this method to persist the bounds at times other than closing the
	 * dialog.
	 * 
	 * @param shell
	 ...
	 * dialog settings. Subclasses should override this method when there are bounds computations
	 * that must be checked each time the dialog is opened.
	 */
	...
	private void applyColors(Composite composite) {
		applyForegroundColor(getShell().getDisplay().getSystemColor(SWT.COLOR_INFO_FOREGROUND),
				composite, getForegroundColorExclusions());
		applyBackgroundColor(getShell().getDisplay().getSystemColor(SWT.COLOR_INFO_BACKGROUND),
	 ...
	 * @param exclusions
	 *           a list of controls who are to be excluded from getting their color assigned
	 */
	 ...
	 * override <code>PopupDialog.getForegroundColorExclusions</code>.
	 * 
	 * @param color
	 *           the color to use as the background color
	 * @param control
	 ...
	 * override <code>PopupDialog.getBackgroundColorExclusions</code>.
	 * 
	 * @param color
	 *           the color to use as the background color
	 * @param control
	 ...
	 * extend this method (should always call <code>super.getForegroundColorExclusions</code> to
	 * aggregate the list.
	 * 
	 * 
	 ...
	 * extend this method (should always call <code>super.getBackgroundColorExclusions</code> to
	 * aggregate the list.
	 * 
	 * @return the List of controls
	 */
	protected List getBackgroundColorExclusions() {
		...
		if (applyAreaComposite != null) {
			DeployUIWidgetFactory.INSTANCE.addWithChildren(applyAreaComposite, list);
		}
		return list;
	}
	...
	private void initializeWidgetState() {
		...
		if (showDialogMenu && showPersistAction) {
			IDialogSettings settings = getDialogSettings();
			if (settings != null) {
				persistBounds = settings.getBoolean(getClass().getName() + DIALOG_USE_PERSISTED_BOUNDS);
			}
		}
