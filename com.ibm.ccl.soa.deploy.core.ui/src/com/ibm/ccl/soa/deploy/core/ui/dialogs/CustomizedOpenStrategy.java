 ...
 * </p>
 */
public class CustomizedOpenStrategy {
	/**
	...
	/**
	 * A single click will generate an open event and key arrows will generate an open event after a
	 * small time.
	 * 
	 * @deprecated
	 */
	...
	/*
	 * SINGLE_CLICK or DOUBLE_CLICK; In case of SINGLE_CLICK, the bits SELECT_ON_HOVER and
	 * ARROW_KEYS_OPEN my be set as well.
	 */
	private int currentMethod = DOUBLE_CLICK;

	private Listener eventHandler;

	private final ListenerList openEventListeners = new ListenerList();

	private final ListenerList selectionEventListeners = new ListenerList();

	private final ListenerList postSelectionEventListeners = new ListenerList();

	/**
	 * @param control
	 *           the control the strategy is applied to
	 * @param method
	 *           One of {@link #SINGLE_CLICK} or {@link #DOUBLE_CLICK}.
	 */
	public CustomizedOpenStrategy(Control control, int method) {
	 ...
	 */
	public void setOpenMethod(int method) {
	...
	/*
	 * Adds all needed listener to the control in order to implement single-click/double-click
	 * strategies.
	 */
		...
		eventHandler = new Listener() {
			...
			public void handleEvent(final Event e) {
				...
				case SWT.MouseExit:
					...
					break;
				case SWT.MouseMove:
					if ((currentMethod & SELECT_ON_HOVER) == 0) {
						return;
					}
					if (e.stateMask != 0) {
						return;
								...
								display.timerExec(TIME, new Runnable() {
									...
									public void run() {
										if (id == count[0]) {
											...
											if ((currentMethod & ARROW_KEYS_OPEN) != 0) {
												fireOpenEvent(new SelectionEvent(e));
											}
										}
									}
			...
			void mouseSelectItem(SelectionEvent e) {
				if ((currentMethod & SINGLE_CLICK) != 0) {
					fireOpenEvent(e);
				}
				mouseUpEvent = null;
				selectionPendent = null;
			...
			void setSelection(Event e) {
				...
				/*
				 * ISSUE: May have to create a interface with method: setSelection(Point p) so that
				 * user's custom widgets can use this class. If we keep this option.
				 */
