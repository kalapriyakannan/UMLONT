...
import com.ibm.ccl.soa.deploy.core.ui.internal.commands.ReorientHostingLinkCommand;
 ...
 * 
 */
public class HostingLinkEditHelper extends DeployCoreBaseEditHelper {

	/**
	 * Gets a command to determine the container for a new hosting link element.
	 */
	protected ICommand getEditContextCommand(final GetEditContextRequest req) {
		...
		if (editRequest instanceof CreateRelationshipRequest) {
			...
			// can only start a host from a unit that can be hosted
...
//				if (!dvs.canBeHosted(unit).isOK()) {
				if (!dvs
						.canBeLinkTarget(unit, new LinkType[] { LinkType.HOSTING, LinkType.CONSTRAINT })
						.isOK()) {
					return null;
				}
			}
	...
	protected ICommand getReorientRelationshipCommand(ReorientRelationshipRequest req) {
		...
		return new ReorientHostingLinkCommand(req);
	}

	/*
	 * configure hosting link (does nothing right now)
	 */
	protected ICommand getConfigureCommand(final ConfigureRequest req) {
		return new ConfigureBaseCommand(req, CorePackage.eINSTANCE.getHostingLink()) {
			protected CommandResult doExecuteWithResult(IProgressMonitor monitor, IAdaptable info) {
				HostingLink link = (HostingLink) req.getElementToConfigure();
