 ...
 * @author Ed Snible
 */
public class EnumerationConstraintUIHandler extends AttributeValueUIHandler {

	/**
	 * 
	 */
	public EnumerationConstraintUIHandler() {
		// do nothing
	}
	...
	public boolean isUIHandlerForDeployModelObject(DeployModelObject dmo) {
		return dmo.eClass() == ConstraintPackage.eINSTANCE.getEnumerationConstraint();
	}

	public DmoComposite createControls(Composite parent, DeployModelObject editDmo,
			FormToolkit formToolkit) /* throws SAFException */{

		EnumerationConstraintValueComposite composite = new EnumerationConstraintValueComposite(
				parent, (EnumerationConstraint) editDmo, formToolkit);
		formToolkit.adapt(composite);
		//		composite.setBackground(Display.getDefault().getSystemColor(SWT.COLOR_MAGENTA));
		return composite;
	}

	private static class EnumerationConstraintValueComposite extends
			AttributeValueUIHandler.AttributeValueComposite {

		private SingleValueListComposite slc;
		private EnumerationConstraint enumConstraint;
		private boolean mutable;

		protected EnumerationConstraintValueComposite(Composite parent, EnumerationConstraint enumc,
				FormToolkit formToolkit) {
			super(parent, enumc, formToolkit);

			setInput(enumc);
			enumConstraint = enumc;
		}

		protected void synthFields(List<EStructuralFeature> synthExclusions) {
			super.synthFields(synthExclusions);

			addEnumerationSection();
		}

		/** 
		 */
		private void addEnumerationSection() {

			slc = new SingleValueListComposite(this, enumConstraint.getValues(), mutable,
					getWidgetFactory(), TransactionUtil.getEditingDomain(enumConstraint), dmo);
		...
		public void setInput(Object object) {
			assert object instanceof EnumerationConstraint;
			super.setInput(object);
			enumConstraint = (EnumerationConstraint) object;
			mutable = DeployModelObjectUtil.isMutable(enumConstraint);

			if (slc != null) {
				slc.setInput(enumConstraint.getValues(), mutable);
			}
		}

	} // end static inner class EnumerationConstraintValueComposite

	private static class SingleValueListComposite extends Composite {

		private static final String DESCR_COL = "Description"; //$NON-NLS-1$
